...
3. **Validate** — `$aquarium:epic-validator` re-checks a completed epic from a clean start and fixes the gaps it confirms. `$aquarium:independent-review` gives staged changes, commits, ranges, tasks, epics, and special investigations one canonical static Codex review contract. `$aquarium:orca-review` applies the same contract through a fresh Orca-managed Codex worker. Aquarium checks every returned finding locally.
