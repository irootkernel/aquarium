...
The user-local `aquarium-dev <tool> [args...]` launcher accepts only supported tools, prefers each available `~/.aquarium-dev/bin` generation, and falls back only a missing tool to the caller's global `PATH` outside both Aquarium roots. If both required generations are absent, it fails closed and requests `$aquarium:dev-setup`; invalid selected development state also fails closed. Podway, Mulgae, Gaori, and Dolgorae require a global binary for production fallback, while Sanho is explicitly optional. Aquarium does not create or own a Codex home, authentication, plugin configuration, or MCP configuration for this channel. Development artifacts are local integration evidence only, not release or distribution proof.
