...
3. **Validate** — `$aquarium:epic-validator` re-checks a completed epic from a clean start and fixes the gaps it confirms. Embedded Task, Epic, and validation review defaults to Mulgae, while an approved workflow may select Orca, a fresh native Codex subagent, or an explicit coordinator waiver; only the selected route's prerequisites apply, and there is no automatic fallback. `$aquarium:mulgae-review` runs one report-only standalone Mulgae review for an exact change or Task or Epic completion target. `$aquarium:orca-review` runs the reviewer you requested against a staged, HEAD, commit, or range target in the current Orca worktree. `$aquarium:independent-review` is temporarily disabled, and its refusal launches nothing. A native Codex subagent carries only the lifecycle guarantees the host provides. Aquarium checks every returned finding locally and keeps technical verdicts separate from completion assessment.
...
- [Mulgae](https://github.com/irootkernel/mulgae) gives completed tasks and epics, plus explicitly requested standalone targets, an advisory multi-provider review from an immutable capture exposed through an isolated read-only provider workspace. Temporary provider workspaces are removed after use. Durable captures and reports remain under `.mulgae/`, and results are returned through native published artifacts. Aquarium supplies the shared purpose and Review Brief, then verifies and reprioritizes each finding locally without rewriting Mulgae lifecycle state.
...
- [Dolgorae](https://github.com/irootkernel/dolgorae) remains an optional global tool for explicitly requested native operations such as Profiles and reusable External Specialist Engagements. Aquarium can diagnose compatible local v0.1.x Apple Silicon installations starting at v0.1.2 and install checksum-verified official stable releases in that range, but the Aquarium Independent Review entrypoint is temporarily disabled and does not discover, set up, or invoke Dolgorae.
- [Orca Review](plugins/aquarium/skills/orca-review/SKILL.md) uses the separately installed Orca runtime to launch and supervise the reviewer you requested, including Claude, in the current registered worktree. It accepts staged, HEAD, commit, and range targets; workspace and dirty targets are unavailable. A staged review reads `git diff --cached` directly. Reviewers receive the shared purpose and Review Brief, return purpose-specific findings and requirement assessments, and must preserve source files, tracked and non-ignored worktree files, and Git state. All reviewers may write review-related temporary files, native state, tool output, and reports outside the worktree or in Git-ignored runtime paths within it, such as ignored files under `.omc/`. External locations include `/tmp` or `/private/tmp`. Permitted external files and ignored runtime writes do not require warnings, extra checks, approval, or another review. Orca Review does not use Dolgorae, and Aquarium independently adjudicates the result.
