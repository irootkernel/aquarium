...
3. **Validate** — `$aquarium:epic-validator` re-checks a completed epic from a clean start and fixes the gaps it confirms. `$aquarium:mulgae-review` runs one report-only standalone Mulgae review for an exact change or Task or Epic completion target. `$aquarium:orca-review` runs the reviewer you requested against a staged, HEAD, commit, or range target in the current Orca worktree. `$aquarium:independent-review` is temporarily disabled and only explains the available alternatives; it launches no review. An explicitly selected native Codex subagent may reuse the same Review Brief when the host actually supports fresh delegation, but it is not an Aquarium review backend and carries only the lifecycle guarantees the host provides. Aquarium checks every returned finding locally and keeps technical verdicts separate from completion assessment.
