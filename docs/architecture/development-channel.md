...
All owned development state is below `~/.aquarium-dev/`: installed manager generations and their private environments in `manager/versions/`, the `manager/current` selector, the `manager/install.lock` installer lock, enrollment records, queues, bounded diagnostics, immutable artifacts, current and pending producer selectors, locks, and opaque producer-owned managed-service runtime. Repositories keep only the producer targets and an exact Aquarium marker inside their native hook. No `.aquarium` repository state is created, and production state below `~/.aquarium/` is not read or changed by this channel.
...
Aquarium does not construct a second Codex environment. The launcher preserves the caller's `CODEX_HOME` and all other environment values except for the intentional PATH prefix. The plugin ships the bundled `aquarium-dev` MCP registration. Authentication, plugin installation settings, and global MCP configuration stay with the caller and their selected Codex home.

Dolgorae may join this development graph as a foreground executable producer only when its repository commits an approved producer implementation and explicitly enrolls that canonical checkout. Until then, the launcher may fall back to the optional global executable for explicit Dolgorae commands. Podway is the first required managed-service producer: its immutable bundle owns the development-safe command and generic controller entrypoints, while Podway retains every LaunchAgent, daemon, socket, registry, and recovery decision. Independent Review is disabled before Dolgorae work, while Orca Review launches the requested native reviewer through Orca and does not use Dolgorae. Podway, Mulgae, and Gaori form the required production global-binary baseline; Dolgorae and Sanho are explicitly optional.
