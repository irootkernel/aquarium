...
| Dolgorae | Explicit setup and delegated operations; the Independent Review route is disabled | Official stable `v0.1.2` through `v0.1.x`; dynamic release identity, machine envelope v2, and v0.1.2-compatible capabilities | Optional native Apple Silicon macOS CLI; same-release `use-dolgorae` skill for delegated operations | Release metadata, archive, installed bytes, machine version, capability digest, and paired-skill readiness remain distinct |
...
Independent Review is temporarily disabled and stops before Dolgorae discovery, setup, source capture, or transmission. It reports explicitly selectable Orca Review and native Codex subagent alternatives without launching either one. A selected native Codex subagent receives the shared Review Brief only through fresh delegation the host actually exposes, retains separate provenance, and carries no implied Dolgorae, Mulgae, or Orca lifecycle guarantees.
