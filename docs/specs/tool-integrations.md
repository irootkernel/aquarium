...
| Dolgorae | Explicit setup and delegated operations; the Independent Review route is disabled | Stable `v0.1.2` through `v0.1.x` | Optional native Apple Silicon macOS CLI; same-release `use-dolgorae` skill for delegated operations | CLI, paired skill, platform, file safety, version, and capabilities remain distinct |
...
| Sanho | Commit inspection and optional documentation synchronization | Stable `v0.2.8` through `v0.2.x` | Matching optional `use-sanho` skill | CLI, skill, workspace enrollment, doctor state, and synchronization authority are independent |
...
| Gaori | Runs existing checks and compresses their output | Stable `v0.1.17` through `v0.1.x` | Repository tester schema v2 when configured | Child exit status is pass/fail authority; parser and summary quality do not create a gate |
...
| Podway | Records Git-backed workflow goals, transitions, handoffs, and session lifecycle | Stable `v0.2.10` through `v0.2.x` | Native Apple Silicon macOS; matching CLI and daemon | CLI, daemon, workspace, managed Procedures, current session, and optional skill are separate readiness axes |
| Ouroboros | Supplies interview, PM, Seed, and QA leaves for explicit design workflows | Stable `>=0.51.1` | Existing `uv`; exact package version for installation | CLI, Codex rules and skills, MCP runtime, effective registration, and live exposure are independent |
...
| Humanizer | Final prose pass for English human-authored documentation | Stable `>=v2.11.1` | Exact detached checkout; upstream skill files at the documented default target, or `~/.agents/skills/humanizer` when no default is named | Installation and project guidance are separate; structural inspection alone cannot prove upstream freshness |
| im-not-ai | Final prose pass for Korean human-authored documentation | Stable `>=v2.3.2` | Exact selected Codex copy materialized in an isolated temporary `CODEX_HOME`, then installed in the active Codex skill root | `_workspace/` is removed after accepted text is applied; meaning and protected content must survive one fail-closed final pass |
...
Independent Review is temporarily disabled and stops before Dolgorae discovery, setup, source capture, or transmission. The refusal launches nothing. Exactly one explicitly preselected supported Orca or native Codex alternative may run only under its own contract; native Codex additionally requires host fresh delegation. Multiple preselected alternatives require the user to choose one before anything launches. A selected native Codex subagent receives the shared Review Brief, retains separate provenance, and carries no implied Dolgorae, Mulgae, or Orca lifecycle guarantees.
...
Dolgorae may separately publish an `aquarium-dev` generation for explicit development commands, but Aquarium creates no production runtime copy and Orca Review does not use Dolgorae.

Aquarium production-binary readiness requires supported global Podway, Mulgae, and Gaori executables. A missing required executable fails closed and routes to the tool-scoped `$aquarium:dev-setup-global` workflow. Dolgorae and Sanho are explicitly optional and do not fail this binary baseline. Development generations never satisfy production readiness.

Mulgae review is advisory. Embedded and standalone callers supply the shared purpose and Review Brief while `$use-mulgae` owns native capture, execution, recovery, publication, and retention. Aquarium requires complete capture coverage, passing CI decision, committed publication, a successful findings query, zero locally verified unresolved findings, and no unmet or unverified completion criterion before approving completion.
...
Orca Review launches one fresh requested native reviewer through Orca in the current registered worktree. It accepts staged, HEAD, commit, and range targets; staged means the live `HEAD`-to-index change read through `git diff --cached`. Workspace and dirty targets are unavailable while Independent Review is disabled. Each Dispatch carries the shared purpose and complete Review Brief. Results keep technical findings, criterion assessment, verification gaps, operational deviations, and native lifecycle state separate; a clean technical result does not establish completion. Tests, authentication, unrequested reviewer changes, source or tracked and non-ignored worktree writes, commits, and publication are outside the static review authorization. All Orca reviewers may create or update review-related temporary files, native session state, tool output, and reports outside the worktree or in Git-ignored runtime paths within it, such as ignored files under `.omc/`. External locations include `/tmp`, `/private/tmp`, or provider-owned directories. They return the paths of retained report files used to deliver the result. Permitted external files and ignored runtime writes do not require warnings, extra checks, approval, or another review.
