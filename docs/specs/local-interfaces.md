...
| --- | --- | --- | --- |
| `aquarium-task-v2` | `12` | `task-handler` | Selected plan input, implementation, refinement, typed verification, and serial finding-confirmation, CI, completion, finding-class with count-consistency guard, explicit rework-authority, phase-owner, user-direction, and finite Low-settlement decisions before evidence-preserving goal assessment, approval, and closeout |
| `aquarium-goal-v2` | `15` | `epic-handler` | One member task, pre-validation remediation, or final closeout goal with serial operational, confirmation, completion, finding-class with count-consistency guard, rework-authority, and Low-handling decisions plus evidence-preserving assessment and a validation-bound closeout substitute |
| `aquarium-validation-v2` | `14` | `epic-handler` and `epic-validator` | Baseline, severity-aware audit, namespaced obligation composition, and serial final-review operation, confirmation, completion, required-evidence, blocker, rework-authority, and Low-settlement decisions with completion evidence preserved through user direction and final assessment |
