...
`plugins/aquarium/skills/aquarium-dev/scripts/aquarium_dev.py` owns diagnose, enrollment, hook repair, build request, rebuild, worker, cleanup, and launcher-installation operations. It has no Codex-configuration or Dolgorae operation. The installed `aquarium-dev` launcher accepts only a supported development executable and arguments, prepends `~/.aquarium-dev/bin` to PATH, executes its exact stable indirection, and rejects missing or unsupported commands without global fallback while leaving the rest of the caller's environment unchanged.
