...
| `release-handler/scripts/inspect_publication_state.py` | `aquarium-release-publication-observation/v2` JSON on standard input | `aquarium-release-publication-state/v2` | Normalizes local, remote, tag, and hosted Release state supplied by the caller |
