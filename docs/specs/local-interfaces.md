...
Humanizer exposes `supported_range: ">=2.11.1"` and accepts a stable frontmatter version at or above that floor. The structural inspector permits additional regular skill files because the selected upstream release owns the exact file set; setup compares the full selected source and target trees. A sole active-home copy is a valid structural candidate, and a sole shared copy remains the fallback. The inspector cannot establish which target the selected upstream release documents. A copy at both targets is a duplicate; copies in inactive Codex homes are independent. Ouroboros reports `legacy_shared_skills` for shared copies matching a selected package skill name and separate same-name `shared_skill_conflicts`; either degrades home readiness.
