...
`plugins/aquarium/skills/aquarium-dev/scripts/aquarium_dev.py` owns diagnose, enrollment, hook repair, build request, rebuild, worker, cleanup, and launcher-installation operations. It has no Codex-configuration or Dolgorae setup operation. The installed `aquarium-dev` launcher accepts only a supported tool and arguments, prefers its exact development generation, falls back only that missing tool to the caller's global `PATH` outside both Aquarium roots, and rejects unknown commands or invalid selected development state while leaving the caller's environment unchanged.
