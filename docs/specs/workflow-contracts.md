...
`new-project`, `new-feature`, `refactor`, `war-room`, and `design-qa` use the shared Ouroboros integration contract and an exact-diff approval boundary. They capture current authority before provider work, treat provider output as draft evidence, and apply repository documents only after the user approves the complete diff.
