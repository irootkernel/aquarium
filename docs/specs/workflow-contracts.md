...
For Independent and Orca reviews, `staged` means the current `HEAD`-to-index change reviewed through `git diff --cached`. Both are static, run no tests or builds, and report adjudicated findings without remediation. Orca reviewers are prohibited from writing in the worktree. All Orca reviewers may retain review-related temporary files, native state, tool output, and reports outside the worktree, including under `/tmp` or `/private/tmp`. They return the paths of retained report files used to deliver the result through the Orca lifecycle. External review files alone do not cause a warning or failed review. These restrictions are workflow contracts for same-user processes, not an operating-system sandbox.
