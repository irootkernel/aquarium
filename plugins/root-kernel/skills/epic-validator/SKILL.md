...
Otherwise read the contract and own one `root-kernel-validation-v2` session for this exact cold-validation and convergence lifecycle. Podway records the audit loop; the roadmap and current implementation remain the semantic authority.
...
6. Honor an explicit pre-session opt-out without Podway discovery and ignore every Podway readiness or session state. Otherwise apply the shared contract's readiness and session checks. Resume only a managed validation session matching this epic and baseline; on degraded readiness or any other active session, stop and ask the user to choose `$root-kernel:dev-setup` repair or an explicit opt-out for this validation.
