...
Never cancel, reset, replace, reopen, or reinterpret the conflicting session automatically. Offer only the dispositions supported by its state and the user's intent: resume unfinished work through its matching owner, leave it untouched through an explicit opt-out for the new workflow, or manage cancellation or deletion through a standalone explicit `$use-podway` request. When deletion or freeing the session slot is desired, provide an exact `$use-podway` current-session discard handoff naming the repository and observed session ID; that handoff grants no cleanup authority by itself.
