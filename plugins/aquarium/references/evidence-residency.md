...
- Orca lifecycle evidence, native Codex delegation evidence, and coordinator waiver records cannot produce a `hardening-deferral` package in the current contract. They expose no Aquarium-authoritative Mulgae target digest, publication, findings query, or exact run membership. Use ordinary Low dispositions or stop on a downstream requirement for promoted native evidence.
