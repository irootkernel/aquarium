...
Every later invocation must use the installed manager's `launch --project-id dolgorae` with all three exact guards: `--expected-git-sha`, `--expected-development-version`, and `--expected-sha256`. The manager leases the immutable generation, opens and rehashes the selected regular executable, compares its device and inode with the resolved path, and keeps both the generation lease and executable descriptor inherited across process replacement. Partial guards, stable fallback, mutable `current` identity, replacement, checksum drift, or enrollment drift fail before source-bearing execution.
