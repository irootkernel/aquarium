...
Every later invocation must use the installed manager's `launch --project-id dolgorae` with all three exact guards: `--expected-git-sha`, `--expected-development-version`, and `--expected-sha256`. Before publication, the Darwin manager creates an owner-only generation-bound hard-link execution alias, then seals the complete generation read-only and user-immutable. Launch leases that generation, opens and rehashes the selected regular executable, compares its device and inode with the canonical artifact, and repeats those checks immediately before executing the alias while keeping both the generation lease and executable descriptor inherited. Manager cleanup clears the flags only while holding the exclusive generation lease. Canonical or alias replacement after verification is rejected by the operating system within the trusted same-user host boundary. Partial guards, stable fallback, mutable `current` identity, replacement, checksum drift, or enrollment drift fail before source-bearing execution. `specialist review`, `review-target capture`, and `review-target settle` reject an omitted guard set as well as a partial one.
