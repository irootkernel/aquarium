...
Tell the user to restart Codex so a new session can expose `start_configured_run`, `start_ad_hoc_run`, `get_run`, `wait_run`, terminal-only `await_run`, `cancel_run`, `get_excerpt`, the read-only `list_runs` completed-evidence inventory, `get_command_stats`, and `estimate_run`. Execution requires the selected start tool and a usable observation path; other tools are optional. Follow `$use-gaori`: prefer terminal awaiting, try it when the host deadline is unknown, and preserve the same invocation and pending host handle. A verified short deadline or observed premature host timeout selects bounded `wait_run`; paced `get_run` is the last resort when both wait tools are unusable. Ending an observer does not cancel the command. `list_runs` is stateless and cannot recover an invocation ID or reattach a disconnected run.

Use `$use-gaori-status` for historical statistics through `get_command_stats` or CLI `gaori --json runs stats <command-id>`, and detailed explanations of an already-known live invocation through `estimate_run`. Report Gaori-returned calculations, availability, selectors, and sample limits without recomputing statistics or inferring success probability. These tools are read-only and grant no acceptance. During execution, `$use-gaori` permits one user-requested `estimate_run` on the same-session invocation while retaining the pending await; the status skill's absence does not prevent that query or execution. Setup never queries run history or estimates automatically.
