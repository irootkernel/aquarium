...
For an explicit reusable Specialist request, follow `$use-dolgorae` and the [Dolgorae consumer contract](dolgorae-review-contract.md#reusable-specialist-requests). General Run and Brokered Hierarchy operations still require their own advertised availability; the supported External Specialist facade retains its separate milestone contract.

## Mulgae

Official source: `https://github.com/irootkernel/mulgae`

Supported releases: exact stable `v0.1.21` and `v0.1.22`, native Apple Silicon macOS only. Resolve the newest non-draft, non-prerelease release in that set. Use the same exact tag for the CLI and its optional `use-mulgae` skill. Mulgae v0.1.20 lacks the command envelope and provider rate-limit contract required by Aquarium. Releases after v0.1.22 require a separately verified contract update; do not accept an arbitrary later v0.1.x schema or cross into `v0.2+`. Installation requires Go `1.26.6` or newer.
...
The binary does not install the agent skill. Default setup diagnosis uses only `command -v mulgae`, `mulgae version --json`, and `mulgae doctor --output json`, plus effective Codex MCP inspection when available. Select the exact response contract from the observed supported version:

| Mulgae | Command envelope | Doctor | Preflight | Configuration | Provider inventory order |
| --- | --- | --- | --- | --- | --- |
| `v0.1.21` | `mulgae-command-result.v8` | `mulgae-doctor-result.v2` | `mulgae-review-preflight.v3` | Config v3 | Kimi, ZCode, AGY, Codex |
| `v0.1.22` | `mulgae-command-result.v11` | `mulgae-doctor-result.v5` | `mulgae-review-preflight.v5` | Config v4 | ZCode, Grok, Codex |

Reject a response that does not match the observed release's row. Do not treat backward-readable envelopes as current responses. If the expected Doctor contract is absent, report the capability as unsupported; never fabricate failed dimensions or reconstruct them from `.mulgae/config.yaml` or `.mulgae/local.yaml`.

Both supported Doctor contracts report `config_v3`, `local_configuration`, `provider_identity`, `configured_readiness`, and `role_route_readiness` independently. The `config_v3` field name remains present in Doctor v5 and does not identify the project's Config version. Preserve each Doctor's `verified`, `failed`, `unverifiable`, and `not_applicable` states and each configured `provider_inventory[]` row's `binary_available` and `cli_compatible` fields. Doctor v5 also requires and reports `application_compatible` for every provider row. Preserve that field separately because application compatibility and CLI or protocol compatibility are independent native decisions. Treat an applicable compatibility result as ready only when its `eligibility` is `eligible`; `newer_than_verified` remains ready under that eligibility. Treat setup as configured only when `configured_readiness.state=ready` and `exit_code=0`. Do not gate or report setup on static evidence, heartbeat, historical reviews, or `review_qualified`, and never report native homes, executable paths, credential-profile homes, credentials, diagnostic messages, request IDs, timestamps, or raw provider output.
...
Mulgae configuration has two authorities. `.mulgae/config.yaml` is Git-shareable project policy; `.mulgae/local.yaml` is untracked mode-`0600` machine configuration. Keep `execution.workspace_access: none`. Load the installed same-release `$use-mulgae` authoring guidance before proposing configuration and ask which providers and roles to configure. For v0.1.21, Config v3 supports Kimi, ZCode, AGY, and Codex; automatic selection requires authenticated ZCode, ZCode protocol version 0.16.5 or newer, and Codex 0.149.0 or newer when selected. For v0.1.22, Config v4 supports ZCode, Grok, and Codex; automatic selection requires both ZCode and Grok, ZCode app 3.12.3, Grok 1.0.30 or newer, and Codex 0.154.0 or newer when selected. Bare initialization enables only the required `logic` role. Consume the selected Doctor's `cli_compatible` and, for Doctor v5, `application_compatible` results instead of reproducing Mulgae's version decisions. Show discovered executable, app-bundle, launcher, data-home, and credential-home paths only in the exact private setup proposal, never in the diagnostic report.

For a new project, run `mulgae init --output json` with every intended provider and role only after approval; it creates the selected release's Config pair and does not edit Git ignore state. Mulgae v0.1.21 writes `validation.extraction.enabled: true` for prose-first structured finding extraction. When a clone contains only shared `config.yaml`, plain `mulgae init --output json` bootstraps only `local.yaml` and rejects project-policy options.
...
Config v1 and v2 are unsupported. Mulgae v0.1.22 also rejects Config v3 and does not migrate it automatically. Show the exact legacy files without reading or printing their contents and apply the shared backup policy before any approved replacement or removal. Never migrate configuration merely because setup diagnosis found a release mismatch.
...
For several Codex identities, keep the two authorities separate using the selected release's Config v3 or Config v4 shape; it is sufficient for the active setup session even when a newly replaced `$use-mulgae` skill will not load until Codex restarts:
...
Show the complete diff and target scope before approval; for a local target also show whether `.codex/config.toml` is tracked. Verify three views independently without starting the server: query the global registration from a neutral filesystem-root cwd with the active Codex home, query the local registration from the same neutral cwd with `CODEX_HOME=<absolute-git-root>/.codex`, and query the effective registration from the repository with the active Codex home. A failed named lookup may use a successful `codex mcp list --json` inventory within the same timeout budget. The supported inventory shape is a top-level array whose entries each have a unique non-empty string `name`; absence from one such inventory proves `missing`, presence means the named lookup is degraded, and a timed-out lookup or failed, malformed, or duplicate-name inventory is unverifiable. Never classify registration state from stderr wording.
...
Run `ooo mcp doctor --json` only for a direct selected-CLI registration. For an isolated launcher, report configured runtime state from its own launcher contract without probing the unrelated base MCP environment. These local probes do not start an MCP server, contact a provider, authenticate, or expose credentials. Normalize errors and reason codes; never print raw registration output or secrets. Only a valid structural inventory may prove absence after a failed named lookup; malformed, disabled, timed-out, or otherwise unresolved probes are degraded, and an absent Codex CLI is unverifiable.
