...
2. Read the roadmap entry, parent epic, linked active dossier, `docs/README.md`, role owners, required artifacts, and [design-gates.md](../../references/design-gates.md). Resolve document paths from the roadmap instead of asking the user. Treat a member task without its required active dossier as a gap and require an explicit `$aquarium:docs-setup` `adopt` retrofit first. Resolve the effective `Design Gate impact` from the task first and then its parent epic. Apply the legacy registry rule; for an enrolled registry, a missing marker is a gap.
...
- In rounds one through three, supply `remediation-eligible` and stop immediately on approval. Record `ci-failed` when the complete review's CI decision is `fail`, `implementation-changes` for a valid implementation or refinement finding after CI passes, or `documentation-changes` for documentation-only findings after CI passes.
  Rework to the route's owning phase, fix every valid in-scope finding there, rerun invalidated evidence, and review the latest complete target with the next ordinal. `$aquarium:task-review` never performs these fixes. An incomplete run stops without recording a review decision or blind retry.
