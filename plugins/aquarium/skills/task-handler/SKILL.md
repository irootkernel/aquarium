...
Always read [evidence-residency.md](../../references/evidence-residency.md), [release-notes.md](../../references/release-notes.md), and [epic-execution-sot.md](../../references/epic-execution-sot.md). Use Podway by default. Exclude it only when the user explicitly opts this task out before its managed session starts or a higher-priority instruction prohibits it. For an opted-out task, do not inspect Podway, load `$use-podway`, or read [podway-integration.md](../../references/podway-integration.md), and do not carry the opt-out into a later workflow.
