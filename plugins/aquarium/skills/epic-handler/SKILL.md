...
7. Honor an explicit pre-session opt-out without Podway discovery. Otherwise apply the shared contract's readiness and session checks. A matching recoverable Aquarium session becomes part of the plan; on degraded readiness or any other active session, stop and ask the user to choose `$aquarium:dev-setup` repair or an explicit opt-out for this epic.
