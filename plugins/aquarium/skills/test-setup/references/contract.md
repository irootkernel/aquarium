...
Additional named gates remain valid, but the aggregate must place them inside the closest common stage. A target that has no applicable subject may succeed with a clear message only when `TESTING.md` gives objective evidence for `not applicable`; it must not hide missing coverage.
