...
Treat the neutral entry as absent only when its entire stderr is Codex's exact named-server-not-found diagnostic. Timeout, mixed diagnostics, malformed output, and every other nonzero result leave project origin unverifiable. Repository configuration paths must be present regular non-symlink paths before any owning CLI probe may read them; an absent path stops that probe rather than consulting ambient state.
