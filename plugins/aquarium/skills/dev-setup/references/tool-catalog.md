...
Supported release: exact `v0.1.0` for native Apple Silicon macOS. The tag peels to source commit `47c95d0d060d9ee685a01bedbdeb5379515e2804`. Do not automatically admit another patch, minor, prerelease, source build, or development generation as the stable review runtime.

The official archive is `dolgorae-v0.1.0-aarch64-apple-darwin.tar.gz` with SHA-256 `598ffaff7883b4f8cc794b890186d758795f57c1f373e6905c138abb7f3bfe41`. Its expected entries are only the top-level directory, `README.txt`, and regular `bin/dolgorae`; the contained executable SHA-256 is `6087b484cfd8d61d88ed69a5b84ab4a515ba2efaebe4fa282d51679536cccdb8`.

Selection authorizes only the bounded official GitHub Release metadata lookup. Obtain separate approval before downloading the archive or checksum asset and another exact approval before installing or replacing the executable. Extract into ephemeral storage, reject absolute or parent-traversing entries, symlinks, hard links, devices, and unexpected files, verify both checksums, and propose one absolute user-owned target, normally `~/.local/bin/dolgorae`. Apply the active backup policy before replacement and never use `sudo` or write a system location.

The v0.1.0 Integration Preview is ad-hoc linker-signed, not Developer ID signed or notarized. Disclose that Gatekeeper may require the user to approve the exact verified binary locally. Verify the installed target as an executable regular non-symlink arm64 Mach-O file, rerun its checksum, and require one successful closed machine envelope from `dolgorae --version` whose command is `version` and data is exactly `{"text":"dolgorae 0.1.0"}`.
