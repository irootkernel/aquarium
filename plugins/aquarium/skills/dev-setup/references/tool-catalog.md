...
Treat the neutral entry as absent only for Codex's exact named-server-not-found diagnostic. Timeout, malformed output, and every other nonzero result leave project origin unverifiable.
...
Treat the neutral entry as absent only for Codex's exact named-server-not-found diagnostic. Timeout, malformed output, and every other nonzero result leave project origin unverifiable. Repository configuration paths must be regular non-symlink paths before any owning CLI probe may read them.
