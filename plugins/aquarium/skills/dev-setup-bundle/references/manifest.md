...
Every effective tool keeps its v1 meaning. The `gaori` selection includes independent global diagnosis of `use-gaori` and `use-gaori-status`; it adds no manifest field or execution prerequisite. `dev-setup-bundle` sends the union of user-global portions to `dev-setup-global` once, then sends each target's repository portions to `dev-setup`. Sorage CLI, paired-skill, and initialization are global. An effective `sorage` selection counts as that target's explicit Project setup request, while Project listing and any approved `project add` or `project bind` action remain target-specific and separately authorized. The manifest carries no Project name, slug, Vault path, or Sorage configuration.
