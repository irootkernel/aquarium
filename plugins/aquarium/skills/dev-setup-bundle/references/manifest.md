...
Every effective tool keeps its v1 meaning. `dev-setup-bundle` sends the union of user-global portions to `dev-setup-global` once, then sends each target's repository portions to `dev-setup`. Sorage CLI, paired-skill, and initialization are global; Project resolution and any approved `project add` or `project bind` action remain target-specific. The manifest carries no Project name, slug, Vault path, or Sorage configuration.
