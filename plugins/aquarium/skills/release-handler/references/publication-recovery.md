...
- current local and live remote `main` SHAs plus the verified remote relationship to the QA candidate;
...
Send only those bounded facts as one `aquarium-release-publication-observation/v2` JSON object through non-expanding stdin to `scripts/inspect_publication_state.py`. Do not interpolate observations into a shell command.

The object contains `version`, `qa_candidate_sha`, `release_commit` (`sha`, `parent_sha`, `title`), `qa_evidence_candidate_sha`, `gate_evidence_release_commit_sha`, `local_main_sha`, `remote_main_sha`, and `remote_main_relation_to_qa_candidate` (`equal`, `ancestor`, `descendant`, or `diverged`). It also contains `tag` (`state`, `annotated`, `peeled_sha`) and `hosted_release` (`state`, `tag`, `target_sha`).
...
- `push_main` when remote `main` still equals or is a verified ancestor of the QA candidate;
