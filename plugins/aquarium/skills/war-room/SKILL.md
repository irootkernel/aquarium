...
After approval, use installed upstream `$interview` and `$qa` as needed. Classify the result as one bounded task, one multi-work-unit epic, or investigation incomplete. Include scope, evidence, root cause or hypotheses, acceptance, dependencies, risks, and `Design Gate impact` on every implementation task. A multi-work-unit epic includes one scope-local active dossier recorded in the adopted TODO index and linked as `Detailed SOT`.

A bounded task added to an existing epic updates its dossier; when it gives a task-less placeholder its first task, create and declare the dossier, record it in the adopted TODO index, and link it as `Detailed SOT`. Gate candidates remain `Pending` for `$aquarium:design-qa`.
