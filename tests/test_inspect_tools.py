...
class InspectToolsTest(unittest.TestCase):
    ...
    def dolgorae_capabilities(self, version: str = "0.1.2") -> dict[str, object]:
        return {
            "dolgorae_version": version,
            ...
            "controller_credential": {
                ...
                "schema_version": 1,
                "schema_sha256": "1" * 64,
                "accepted_kinds": ["workflow_orchestrator"],
            ...
            "artifact_bounds": {
                "maximum_artifact_bytes": 33_554_432,
                "maximum_chunk_bytes": 1_048_576,
                "maximum_inline_response_bytes": 1_048_576,
                "digest": "sha256",
            ...
            "control_modes": ["managed_agent"],
            "execution_lanes": ["shared_readonly"],
            "lane_capabilities": {
                "shared_readonly": {
            ...
            "interactions": {
                ...
                "maximum_safe_payload_bytes": 8_388_608,
            },
        }

    def write_fake_dolgorae(
        self,
...
if sys.argv[1:] == ["version", "--json"]:
    print(json.dumps({{"name": "dolgorae", "version": "v{version}"}}))
    raise SystemExit(0)
elif sys.argv[1:] == ["runtime", "capabilities"]:
    data = {capabilities!r}
    command = "runtime.capabilities"
else:
    raise SystemExit(2)
print(json.dumps({{"schema_version": 2, "ok": True, "command": command, "invocation_id": "019d0000-0000-7000-8000-000000000000", "data": data}}))
""",
            encoding="utf-8",
        )
        ...
        source = textwrap.dedent(
            ...
            if name == "mulgae":
                ...
                if arguments == ["doctor", "--output", "json"]:
                    ...
                    application_required = {inspect_tools.numbered_schema_at_least(mulgae_doctor_schema, "mulgae-doctor-result.v", 5)!r}
    ...
    def test_dolgorae_rejects_invalid_version_json(self) -> None:
        ...
        cases = [
            {"name": "other", "version": "v0.1.2"},
            {"name": "dolgorae"},
            valid | {"extra": True},
            {"name": "dolgorae", "version": 2},
            {"name": "dolgorae", "version": "0.1.2"},
            {"name": "dolgorae", "version": "vv0.1.2"},
            {"name": "dolgorae", "version": "v0.01.2"},
            {"name": "dolgorae", "version": "v0.1.2+"},
            {"schema_version": 1, "ok": True, "data": {"text": "dolgorae 0.1.1"}},
        ]
        outputs = [(json.dumps(case), 0, "") for case in cases]
        outputs.extend(
            [
                (json.dumps(valid) + json.dumps(valid), 0, ""),
                ('{"name":"other","name":"dolgorae","version":"v0.1.2"}', 0, ""),
                (json.dumps(valid), 5, ""),
                (json.dumps(valid), 0, "unexpected diagnostic"),
                ("dolgorae v0.1.2\n", 0, ""),
            ]
        )
        real_run = inspect_tools.run_command
        for output, exit_code, stderr in outputs:
            with self.subTest(output=output, exit_code=exit_code, stderr=stderr):
    ...
    def test_dolgorae_inspection_detects_executable_drift_after_probes(self) -> None:
        ...
        def run_and_replace(
            *arguments: object, **keywords: object
            ...
            result = real_run_command(*arguments, **keywords)
            probe_count += 1
            if probe_count == 2:
        ...
        with (
            ...
            mock.patch.object(inspect_tools.platform, "machine", return_value="arm64"),
            mock.patch.object(
                inspect_tools, "run_command", side_effect=run_and_replace
            ),
        ):
            tool = inspect_tools.inspect_dolgorae(
                self.repository, NORMAL_PROBE_TIMEOUT_SECONDS
    ...
    def test_dolgorae_v010_and_prereleases_are_not_supported(self) -> None:
        ...
        self.assertFalse(inspect_tools.supported_dolgorae_version("v0.1.1"))
        self.assertFalse(inspect_tools.supported_dolgorae_version("v0.1.3-rc.1"))
        self.assertFalse(inspect_tools.supported_dolgorae_version("v0.2.0"))
        self.assertTrue(inspect_tools.supported_dolgorae_version("v0.1.2"))
        self.assertTrue(inspect_tools.supported_dolgorae_version("0.1.99"))

    def test_dolgorae_capability_contract_rejects_incompatible_fields(self) -> None:
        mutations: list[tuple[str, tuple[str, ...], object]] = [
            ("version", ("dolgorae_version",), "0.1.3"),
            ("minimum_rpc_client", ("minimum_rpc_client_version",), 2),
            ("maximum_rpc_client", ("maximum_rpc_client_version",), 0),
            ("rpc_descriptor", ("rpc_descriptor_sha256",), "invalid"),
            (
                "carrier_root",
                ("controller_carrier_root",),
                "home/.dolgorae/other",
            ),
            ("transport", ("supported_transports",), []),
            ("launch_mode", ("profile_launch_mode",), "other"),
            ("control_mode", ("control_modes",), []),
            ("execution_lane", ("execution_lanes",), []),
            (
                "writer_support",
                ("lane_capabilities", "shared_readonly", "writer_support"),
                True,
            ),
            (
                "codex_mode",
                ("lane_capabilities", "shared_readonly", "codex_mode"),
                "default",
            ),
            (
                "command_execution",
                ("lane_capabilities", "shared_readonly", "command_execution"),
                "unbounded",
            ),
            ("credential_object", ("controller_credential",), {}),
            (
                "credential_schema_id",
                ("controller_credential", "schema_id"),
                "other",
            ),
            (
                "credential_schema_version",
                ("controller_credential", "schema_version"),
                2,
            ),
            (
                "credential_schema_digest",
                ("controller_credential", "schema_sha256"),
                "invalid",
            ),
            (
                "credential_byte_length",
                ("controller_credential", "capability_byte_length"),
                31,
            ),
            (
                "credential_encoding",
                ("controller_credential", "capability_encoding"),
                "base64",
            ),
            ("credential_same_uid", ("controller_credential", "same_uid"), False),
            (
                "credential_regular_file",
                ("controller_credential", "regular_file"),
                False,
            ),
            (
                "credential_symlinks",
                ("controller_credential", "symlinks"),
                "allowed",
            ),
            (
                "credential_exclusive",
                ("controller_credential", "create_exclusive"),
                False,
            ),
            (
                "credential_maximum_bytes",
                ("controller_credential", "maximum_file_bytes"),
                0,
            ),
            (
                "credential_descendant_pattern",
                ("controller_credential", "client_descendant_pattern"),
                "other",
            ),
            (
                "credential_principal",
                ("controller_credential", "normalized_principal"),
                "other",
            ),
            (
                "credential_generation",
                ("controller_credential", "initial_generation"),
                0,
            ),
            (
                "credential_kind",
                ("controller_credential", "accepted_kinds"),
                [],
            ),
            ("artifact_bounds_object", ("artifact_bounds",), {}),
            ("artifact_digest", ("artifact_bounds", "digest"), "other"),
            (
                "artifact_exact_length",
                ("artifact_bounds", "exact_byte_length"),
                False,
            ),
            (
                "artifact_maximum",
                ("artifact_bounds", "maximum_artifact_bytes"),
                0,
            ),
            (
                "artifact_chunk_maximum",
                ("artifact_bounds", "maximum_chunk_bytes"),
                0,
            ),
            (
                "artifact_inline_maximum",
                ("artifact_bounds", "maximum_inline_response_bytes"),
                0,
            ),
            (
                "artifact_visibility",
                ("artifact_bounds", "visibility_classes"),
                ["observer"],
            ),
            ("features_object", ("features",), {}),
            ("interactions_object", ("interactions",), {}),
            (
                "interaction_response_maximum",
                ("interactions", "maximum_response_bytes"),
                0,
            ),
            (
                "interaction_payload_maximum",
                ("interactions", "maximum_safe_payload_bytes"),
                0,
            ),
        ]
        mutations.extend(
            (f"protocol_{field}", (field,), 2)
            for field in (
        ...
        mutations.extend(
            (f"feature_{field}", ("features", field), False)
            for field in (
                "controller_binding",
                "operator_capability",
                "operator_controller_reset",
        ...
        self.assertFalse(inspect_tools.dolgorae_capabilities_compatible([], "0.1.2"))
        for name, path, replacement in mutations:
            with self.subTest(name=name):
                capabilities = copy.deepcopy(self.dolgorae_capabilities())
                ...
                self.assertFalse(
                    inspect_tools.dolgorae_capabilities_compatible(
                        capabilities, "0.1.2"
                    )
                )
        ...
        for path in integer_fields:
            for wrong_type in (True, 1.0):
                with self.subTest(path=path, wrong_type=wrong_type):
                    capabilities = copy.deepcopy(self.dolgorae_capabilities())
                    container = capabilities
                    for component in path[:-1]:
                        container = container[component]
                    expected = container[path[-1]]
                    container[path[-1]] = (
                        float(expected) if type(wrong_type) is float else wrong_type
                    )
                    self.assertFalse(
                        inspect_tools.dolgorae_capabilities_compatible(
                            capabilities, "0.1.2"
        ...
        widened_rpc_range = self.dolgorae_capabilities()
        widened_rpc_range["maximum_rpc_client_version"] = 2
        self.assertTrue(
            inspect_tools.dolgorae_capabilities_compatible(widened_rpc_range, "0.1.2")
        )

    def test_dolgorae_machine_envelope_rejects_structural_drift(self) -> None:
        envelope = {
            "schema_version": 2,
            "ok": True,
            "command": "runtime.capabilities",
            "invocation_id": "019d0000-0000-7000-8000-000000000000",
            "data": self.dolgorae_capabilities(),
        }
        raw_probe = {
            "attempted": True,
            "ok": True,
        ...
        self.assertTrue(
            inspect_tools.valid_dolgorae_envelope(
                probe, raw_probe, "runtime.capabilities"
            )
        )

        mutations = {
            "unknown key": lambda item: item.__setitem__("extra", True),
            "schema": lambda item: item.__setitem__("schema_version", 1),
            "float schema": lambda item: item.__setitem__("schema_version", 2.0),
            "not ok": lambda item: item.__setitem__("ok", False),
            "command": lambda item: item.__setitem__("command", "other"),
            "invocation": lambda item: item.__setitem__("invocation_id", "invalid"),
        }
        for name, mutate in mutations.items():
            with self.subTest(name=name):
                changed_probe = copy.deepcopy(probe)
                mutate(changed_probe["result"])
                self.assertFalse(
                    inspect_tools.valid_dolgorae_envelope(
                        changed_probe, raw_probe, "runtime.capabilities"
                    )
                )

        contaminated = copy.deepcopy(raw_probe)
        contaminated["stderr"] = "unexpected"
        self.assertFalse(
            inspect_tools.valid_dolgorae_envelope(
                probe, contaminated, "runtime.capabilities"
            )
        )
    ...
    def test_dolgorae_inspection_rejects_incompatible_capabilities(self) -> None:
        ...
        self.assertEqual(tool["status"], "degraded")
        self.assertFalse(tool["capabilities_compatible"])
        self.assertEqual(
            tool["probes"]["capabilities"]["error_code"],
    ...
    def test_dolgorae_inspection_preserves_version_probe_failures(self) -> None:
        self.write_fake_dolgorae(self.dolgorae_capabilities())
        for stdout, execution_error, expected_code in (
            ("not-json", None, "invalid_json"),
            ('{"name":"dolgorae"} trailing', None, "invalid_json"),
            ('{"name":"a","name":"b"}', None, "invalid_json"),
            ("{} {}", None, "invalid_json"),
            ("", "execution_failed", "execution_failed"),
            ("{}", None, "unexpected_version_envelope"),
        ):
            with self.subTest(stdout=stdout, execution_error=execution_error):
                raw_probe = {
                    "attempted": True,
                    "ok": execution_error is None,
                    "exit_code": 0 if execution_error is None else None,
                    "timed_out": False,
                    "stdout": stdout,
                    "stderr": "",
                }
                if execution_error:
                    raw_probe["error_code"] = execution_error
                with (
                    mock.patch.dict(os.environ, self.environment, clear=True),
                    mock.patch.object(
...
import sys
if sys.argv[1:] == ["version", "--json"]:
    print(json.dumps({{"name": "dolgorae", "version": "v0.1.2"}}))
elif sys.argv[1:] == ["runtime", "capabilities"]:
    print("not-json")
else:
    raise SystemExit(2)
""",
    ...
    def test_dolgorae_arm64_macho_detection(self) -> None:
        ...
        arm64_dylib = self.base / "arm64-dylib"
        x86_64 = self.base / "x86_64"
        short = self.base / "short"
        arm64.write_bytes(
            b"\xcf\xfa\xed\xfe\x0c\x00\x00\x01\x00\x00\x00\x00\x02\x00\x00\x00"
        )
        arm64_dylib.write_bytes(
            b"\xcf\xfa\xed\xfe\x0c\x00\x00\x01\x00\x00\x00\x00\x06\x00\x00\x00"
        )
        x86_64.write_bytes(
            b"\xcf\xfa\xed\xfe\x07\x00\x00\x01\x00\x00\x00\x00\x02\x00\x00\x00"
        )
        short.write_bytes(b"\xcf\xfa")

        self.assertTrue(inspect_tools.is_arm64_macho(arm64))
        self.assertFalse(inspect_tools.is_arm64_macho(arm64_dylib))
    ...
    def test_prior_canonical_digest_preserves_readiness_and_local_bytes(self) -> None:
        self.install_fake_tools()
        self.install_managed_podway_procedures()
        prior_digests = {
        ...
        file_sha256 = inspect_tools.file_sha256

        def installed_digest(path: Path) -> str | None:
            if path.parent == managed and path.name in prior_digests:
                return prior_digests[path.name]
        ...
        with (
            ...
            mock.patch("inspect_tools.platform.system", return_value="Darwin"),
            mock.patch("inspect_tools.platform.machine", return_value="arm64"),
            mock.patch("inspect_tools.file_sha256", side_effect=installed_digest),
        ):
            podway = inspect_tools.inspect_podway(
                self.repository.resolve(), NORMAL_PROBE_TIMEOUT_SECONDS
            )
        ...
        for entry in podway["managed_procedures"]:
            ...
            if name not in prior_digests:
                continue
            with self.subTest(procedure=name):
                self.assertEqual(entry["update_explanation"], "prior_canonical")
                self.assertEqual(entry["source_state"], "valid_customization")
                self.assertFalse(entry["matches_source"])
                self.assertEqual((managed / name).read_bytes(), before[name])
        self.assertEqual(podway["readiness_status"], "ready")
        self.assertEqual(podway["status"], "configured")
        self.assertFalse(podway["migration_required"])
    ...
    def test_mulgae_version_support_has_a_stable_minimum_without_an_upper_bound(
        self,
    ) -> None:
        cases = (
            ...
            ("v0.1.21", False, "degraded"),
            ("0.1.21", False, "degraded"),
            ("v0.1.22", False, "degraded"),
            ("0.1.22", False, "degraded"),
            ...
            ("v1.0.0", True, "installed"),
            ("v0.1.23-rc.1", False, "degraded"),
            ("v0.1.24-rc.1", False, "degraded"),
            ("v0.2.0-rc.1", False, "degraded"),
            ("v1.0.0-alpha.1", False, "degraded"),
        ...
        for version, supported, status in cases:
            with self.subTest(version=version):
                ...
                if not supported:
                    self.assertEqual(
                        mulgae["health"]["doctor_contract"], "not_observed"
                    )
                    self.assertEqual(
                    ...
                    self.assertEqual(
                        mulgae["health"]["role_route_readiness"]["reason_codes"],
                        ["unsupported_native_version"],
                    )

    def test_mulgae_numeric_contracts_have_no_integer_conversion_limit(self) -> None:
        huge_component = "9" * 4301
        version = f"v{huge_component}.0.0"
        command_schema = f"mulgae-command-result.v{huge_component}"
        doctor_schema = f"mulgae-doctor-result.v{huge_component}"
        ...
        self.assertTrue(
            inspect_tools.numbered_schema_at_least(
                doctor_schema, "mulgae-doctor-result.v", 5
            )
        )
        self.assertTrue(
            inspect_tools.supported_mulgae_go_version(f"go{huge_component}.0.0")
        )
    ...
    def test_mulgae_version_probe_failure_is_not_an_unsupported_version(self) -> None:
        self.install_fake_tools()
        original_json_probe = inspect_tools.json_probe
        version_failure: dict[str, object]

        def fail_mulgae_version_probe(
            arguments: list[str],
            repository: Path,
            ...
            if command == ["mulgae", "version", "--json"]:
                return version_failure.copy()
            if command == ["mulgae", "doctor", "--output", "json"]:
                self.fail("Doctor must not run after the Mulgae version probe fails")
            return original_json_probe(
                arguments, repository, timeout_seconds, environment_overrides
            )
        ...
        for version_failure in failure_cases:
            for configuration_present in (False, True):
                shutil.rmtree(self.repository / ".mulgae", ignore_errors=True)
                if configuration_present:
                    self.install_mulgae_config()
                with self.subTest(
                    error_code=version_failure.get("error_code"),
                    timed_out=version_failure["timed_out"],
                    ...
                    with (
                        ...
                        mock.patch.object(
                            inspect_tools,
                            "json_probe",
                            side_effect=fail_mulgae_version_probe,
                        ),
                    ):
                        mulgae = inspect_tools.inspect_mulgae(
                            self.repository.resolve(), NORMAL_PROBE_TIMEOUT_SECONDS
                    ...
                    self.assertFalse(mulgae["version_supported"])
                    self.assertEqual(
                        mulgae["probes"]["version"]["timed_out"],
                        version_failure["timed_out"],
                    )
                    if "error_code" in version_failure:
                        self.assertEqual(
                            mulgae["probes"]["version"]["error_code"],
                            version_failure["error_code"],
                        )
                    else:
                        self.assertNotIn("error_code", mulgae["probes"]["version"])
                    self.assertEqual(
                        mulgae["health"]["doctor_contract"], "not_observed"
                    )
                    self.assertEqual(
                    ...
                    expected_doctor_reason = (
                        "version_probe_failed"
                        if configuration_present
                        else "configuration_missing"
                    )
                    self.assertEqual(
                        mulgae["probes"]["doctor"]["reason"], expected_doctor_reason
                    )

    def test_mulgae_config_v4_pair_and_private_policy_are_verified(self) -> None:
        self.install_fake_tools()
        self.install_mulgae_config()
        with (
            mock.patch.dict(os.environ, self.environment),
    ...
    def test_mulgae_doctor_v5_dimensions_are_independent_and_observable(
        self,
    ) -> None:
        self.install_fake_tools()
        self.install_mulgae_config()
    ...
    def test_mulgae_supported_doctor_preserves_drifted_config(self) -> None:
        self.install_fake_tools(mulgae_doctor_case="config_drifted")
        self.install_mulgae_config()
        mulgae = json.loads(self.inspect().stdout)["tools"]["mulgae"]
        self.assertEqual(mulgae["status"], "degraded")
    ...
    def test_mulgae_doctor_accepts_additive_bounded_config_reason_codes(self) -> None:
        self.install_fake_tools()
        self.install_mulgae_config()
        raw = subprocess.run(
        ...
        self.assertEqual(
            normalized["result"]["doctor"]["config"]["reason_codes"],
            ["future_config_reason"],
        )

        mixed_envelope = copy.deepcopy(envelope)
        mixed_envelope["result"]["doctor"]["config"]["reason_codes"] = [
        ...
        normalized = inspect_tools.normalize_mulgae_doctor(
            {
                ...
                "exit_code": 8,
                "timed_out": False,
                "result": mixed_envelope,
            },
            contract,
        )
        self.assertEqual(
            normalized["result"]["doctor"]["config"]["reason_codes"],
            ["config_credential_value_detected", "future_config_reason"],
        )

        for invalid_reason in ("", "Uppercase", "has-hyphen", "x" * 65, 42):
            with self.subTest(invalid_reason=invalid_reason):
                invalid_envelope = copy.deepcopy(envelope)
                invalid_envelope["result"]["doctor"]["config"]["reason_codes"] = [
                    invalid_reason
                ]
                normalized = inspect_tools.normalize_mulgae_doctor(
                    {
                ...
                self.assertEqual(normalized["doctor_capability"], "supported")
                self.assertNotIn(
                    "reason_codes",
                    normalized["result"]["doctor"]["config"],
                )

    def test_mulgae_supported_doctor_preserves_unsafe_config_reasons(self) -> None:
        cases = (
        ...
        for doctor_case, reason, locality in cases:
            with self.subTest(doctor_case=doctor_case):
                for executable in self.bin_directory.iterdir():
                    executable.unlink()
                shutil.rmtree(self.repository / ".mulgae", ignore_errors=True)
                self.install_fake_tools(mulgae_doctor_case=doctor_case)
                self.install_mulgae_config()
                if doctor_case == "native_home":
                    ...
                    raw_config = json.loads(raw.stdout)["result"]["doctor"]["config"]
        ...
        for schema in (
            ...
            with self.subTest(schema=schema):
                ...
                self.assertEqual(
                    ...
                    {
                        "status": "unverifiable",
                        "reason_codes": ["unsupported_output_schema"],
                    },
                )
        ...
        for (
            ...
            with self.subTest(version=version, doctor_schema=doctor_schema):
                ...
                if error:
                    ...
                    self.assertEqual(
                        mulgae["health"]["config_v3"]["reason_codes"], [error]
                    )
                    self.assertEqual(
                        mulgae["health"]["configured_readiness"]["reason_codes"],
                        [error],
                    ...
                    self.assertEqual(probe["doctor_capability"], "supported")

    def test_mulgae_global_and_repository_contract_boundaries_are_distinct(
        self,
    ) -> None:
        self.install_fake_tools(
            mulgae_version="v0.1.24",
            mulgae_output_schema="mulgae-command-result.v11",
            mulgae_doctor_schema="mulgae-doctor-result.v4",
        )
        global_mulgae = json.loads(self.inspect_global().stdout)["tools"]["mulgae"]
        self.assertTrue(global_mulgae["cli"]["version_supported"])
        self.assertNotIn("error_code", global_mulgae["cli"]["version_probe"])

        self.install_mulgae_config()
        repository_mulgae = json.loads(self.inspect().stdout)["tools"]["mulgae"]
        self.assertEqual(repository_mulgae["status"], "degraded")
        self.assertEqual(
            repository_mulgae["probes"]["doctor"]["error_code"],
            "unsupported_output_schema",
        )
    ...
    def test_mulgae_future_provider_family_is_preserved(self) -> None:
        ...
        self.assertEqual(
            [row["family"] for row in mulgae["provider_inventory"]],
            ["zcode", "grok", "codex", "future_provider"],
        )

    def test_mulgae_provider_inventory_accepts_native_reorder_and_retirement(
        self,
    ) -> None:
        self.install_fake_tools()
        self.install_mulgae_config()
        ...
        envelope = json.loads(raw.stdout)
        doctor = envelope["result"]["doctor"]
        zcode, _grok, _codex = doctor["provider_inventory"]
        grok = copy.deepcopy(zcode)
        grok["family"] = "grok"
        grok["referenced_by_roles"] = ["security"]
        doctor["provider_inventory"] = [grok, zcode]
        doctor["configured_provider_ids"] = ["grok", "zcode"]
        ...
        self.assertEqual(normalized["doctor_capability"], "supported")
        safe_doctor = normalized["result"]["doctor"]
        self.assertEqual(
            [row["family"] for row in safe_doctor["provider_inventory"]],
            ["grok", "zcode"],
        )
        self.assertEqual(safe_doctor["configured_provider_ids"], ["grok", "zcode"])

        mismatched = copy.deepcopy(envelope)
        mismatched["result"]["doctor"]["configured_provider_ids"] = [
            "zcode",
            "grok",
        ]
        normalized = inspect_tools.normalize_mulgae_doctor(
    ...
    def test_mulgae_provider_inventory_enforces_bounded_canonical_shape(self) -> None:
        diagnostic = {"status": "not_applicable", "reason_codes": []}
        compatibility = {
            "status": "not_applicable",
            "observed_version": "",
            "eligibility": "not_evaluated",
        ...
        def row(family: str, roles: list[str] | None = None) -> dict[str, object]:
            return {
                "family": family,
                "configured": False,
                "referenced_by_roles": roles or [],
                "state": "not_configured",
                "reason": "not_configured",
                "binary_available": diagnostic,
                "cli_compatible": compatibility,
                "application_compatible": compatibility,
            }

        fields = ("cli_compatible", "application_compatible")
        self.assertEqual(
            inspect_tools.normalize_mulgae_provider_inventory(
                [row("future-provider", ["future-role"])], fields
            )[0]["referenced_by_roles"],
            ["future-role"],
        )
        invalid_inventories = (
            [],
            [row("grok"), row("grok")],
            [row("grok", ["logic", "logic"])],
            [row("Grok")],
            [row("has space")],
            [row("x" * 65)],
            [row(f"provider-{index}") for index in range(65)],
        )
        for inventory in invalid_inventories:
            with self.subTest(inventory_size=len(inventory)):
                self.assertIsNone(
                    inspect_tools.normalize_mulgae_provider_inventory(inventory, fields)
                )
        ...
        mulgae = inspect_tools.normalize_mulgae_doctor(
            {
                ...
                "result": {
                    ...
                    "result": {
                        ...
                        "doctor": {
                            "schema_version": "mulgae-doctor-result.v5",
    ...
    def test_mulgae_doctor_v5_provider_outcomes_gate_offline_readiness(self) -> None:
        self.install_mulgae_config()
        cases = (
        ...
        for case, status, binary, eligibility, compatibility in cases:
            with self.subTest(case=case):
                ...
                self.assertEqual(mulgae["status"], status)
                zcode = mulgae["provider_inventory"][0]
    ...
    def test_mulgae_doctor_v5_rejects_invalid_identity_and_role_mapping(self) -> None:
        self.install_mulgae_config()
        for case, reason in (
    ...
    def test_mulgae_legacy_doctor_fields_are_unsupported_not_failed(self) -> None:
        ...
        self.assertEqual(
            ...
            {
                "status": "unverifiable",
                "reason_codes": ["doctor_contract_unsupported"],
            },
        )
        self.assertNotIn("provider_static_admission", mulgae["health"])
        self.assertNotIn("live_review", mulgae["health"])

    def test_mulgae_doctor_v5_missing_required_dimensions_is_invalid(self) -> None:
        normalized = inspect_tools.normalize_mulgae_doctor(
            {
                "attempted": True,
                "ok": True,
    ...
    def test_default_inspection_never_calls_global_skill_inspector(self) -> None:
        ...
        self.assertEqual(
            payload["trusted_global_skills"]["use-dolgorae"],
            {
                "canonical_path": str(skill_root / "use-dolgorae"),
                "present": True,
                "verification_scope": "presence_only",
            },
        )
