...
- Last revalidated: 2026-08-22 after conservative inspector coverage for symlinked configuration and repository ancestry, absent owning configuration, exact neutral MCP diagnostics, quoted and additive pytest control-only settings, token-joined Make aliases, and Delivery acknowledgement was added; the complete local gate passed with the same isolated unittest suites and no new integration boundary.
