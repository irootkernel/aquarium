...
- Last revalidated: 2026-08-22 after conservative inspector coverage for pre-normalization repository and skill-root ancestry, all Gaori owning configuration, absent owning and Mulgae doctor configuration, exact neutral MCP diagnostics, quote-joined pytest controls, quote/backslash-joined Make aliases, terminal-only Orca settlement, consent-bound provider executable identity, and the disclosed same-user CLI boundary was added; the complete local gate passed with the same isolated unittest suites and no new integration boundary.
