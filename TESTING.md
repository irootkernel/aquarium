...
- Last revalidated: 2026-08-22 against functional candidate `2745e9f41c36346ca68731152f41c9a3163faceb`. The complete local gate passed with native pytest coverage for Ginkgo execution modes, field-scoped Python dependency authorities, manifestless Python and TypeScript polyglot attribution, pytest option-aware and node-selector-aware TypeScript-owned Python E2E scope, direct and module legacy runners, Python E2E attribution, nonempty Bun lock evidence, runner authority, and JSON boundaries plus waived unittest coverage for composed commands, executable, escaped, quoted and expanding heredoc substitution markers, nested command-scoped Git routing, chained conditional `cd`, matched `case` branches, and invoked shell-function Git routing, after rechecking the three isolated legacy suites, their temporary-repository and subprocess boundaries, the fixed Make runners, dependency pins, and the unchanged unittest major-version contract.
