...
- Last revalidated: 2026-08-22 after conservative inspector coverage for pre-normalization symlink ancestry, absent owning and Mulgae doctor configuration, exact neutral MCP diagnostics, quoted, multiline, and additive pytest control-only settings, empty and non-empty token-joined Make aliases, and Delivery acknowledgement was added; the complete local gate passed with the same isolated unittest suites and no new integration boundary.
