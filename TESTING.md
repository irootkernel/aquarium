...
- Last revalidated: 2026-08-22 against functional candidate `d25fd7896acef58989fe3713c302625fc0d3ba6b`. The complete local gate passed after extending native pytest coverage for split-command Git routing, non-executing Ginkgo modes, malformed Python configuration, and conditional runner authorities, then rechecking the remaining three isolated unittest suites, their temporary-repository and subprocess boundaries, the fixed Make runners, dependency pins, and the unchanged unittest major-version contract.
