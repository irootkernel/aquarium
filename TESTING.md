...
- Last revalidated: 2026-08-22 after conservative inspector coverage for pre-normalization repository and skill-root ancestry, Gaori rule descendants, symlinked root test authorities, Make continuations and extended aliases, operator-adjacent pytest controls, terminal-only Orca settlement, consent-bound outside-repository provider identity, verified local Orca routing, and the disclosed same-user CLI boundary was added; the complete local gate passed with the same isolated unittest suites and no new integration boundary.
